...
if ($("#stream1").length > 0) {
  ...
  function set_video() {
    ...
    if(new_stream_status == 'live') {
        $('#videostream').empty();
        $('#videostream').append('<iframe width="560" height="315" src="//www.youtube.com/embed/eHuwuDCCvoM?rel=0&amp;showinfo=0" frameborder="0" allowfullscreen></iframe><br/> <a href="/live" type="button" class="btn btn-primary"> Élő közvetítés</a>');
    }else{
        $('#videostream').empty();
